...
namespace Valsom.Logging.PrettyConsole.Formats.Default
{
    /// <summary>
    /// The default console format
    /// </summary>
    public class DefaultConsoleFormat : IConsoleFormat
    {
        internal DefaultConsoleFormat()
        {

        }

        private IConsoleTheme Theme { get; set; }

        /// <inheritdoc />
        public StringBuilder CreateLogEntry(LogLevel logLevel, string category, EventId eventId, string message, Exception ex, IConsoleTheme theme)
        {
            Theme = theme;

            (string levelAsString, Color levelAsColor) = GetLocalizationFromLevel(Theme, logLevel);

            IAnsiStringBuilder stamp = CreateStamp(levelAsString, levelAsColor);
            IAnsiStringBuilder tag = CreateTag(category, eventId, levelAsColor);
            IAnsiStringBuilder sourceTag = CreateSourceTag(eventId, levelAsColor);
            IAnsiStringBuilder content = CreateContent(message, ex, levelAsColor);
            IAnsiStringBuilder exception = CreateException(ex, levelAsColor);

            int indentSize = stamp.BuildClean().Length + 2;
            int maxWidth = System.Console.WindowWidth - indentSize;

            string stampText = stamp.Build();
            string tagText = tag.BuildWithWrap(maxWidth);
            string sourceTagText = sourceTag.BuildWithWrap(maxWidth);
            string contentText = content.BuildWithWrap(maxWidth);
            string exceptionText = exception.BuildWithWrap(maxWidth);
        ...
        private IAnsiStringBuilder CreateException(Exception ex, Color accentColor)
        {
            ...
            while (ex != null)
            {
                ...
                if (ex.Data.Count > 0)
                {
                    ...
                    foreach (DictionaryEntry o in ex.Data)
                    {
                        ...
                        exception.AddPart(": ").With(Theme.Punctuation);
                        exception.AddPart(JsonConvert.SerializeObject(o.Value)).With(Theme.Foreground);
                        exception.AddPart("}").With(Theme.Punctuation);
                        isFirstEntry = false;
                    }
                    exception.AddPart("]").With(Theme.Punctuation);
            ...
            exception.EndWith(Theme.Foreground);
        ...
        private IAnsiStringBuilder CreateContent(string message, Exception exception, Color accentColor)
        {
            //(message)
            ...
            content.EndWith(Theme.Foreground);
        ...
        private IAnsiStringBuilder CreateStamp(string level, Color accentColor)
        {
            ...
            stamp.EndWith(Theme.Foreground);
        ...
        private IAnsiStringBuilder CreateSourceTag(EventId eventId, Color accentColor)
        {
            ...
            {
                tag.AddPart($" {eventId.Name}").With(Theme.Foreground);
            }

            tag.EndWith(AnsiCode.Reset);
            tag.EndWith(Theme.Foreground);
        ...
        private IAnsiStringBuilder CreateTag(string category, EventId eventId, Color accentColor)
        {
            ...
            tag.AddPart(categoryParts.Last()).With(Theme.Foreground);
        ...
        private static (string, Color) GetLocalizationFromLevel(IConsoleTheme theme, LogLevel level)
        {
            switch (level)
            {
                case LogLevel.Trace:
